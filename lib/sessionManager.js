...
const { remember: rememberBotMessage } = require('./bot-messages');
...
function _routeMessage(message, sock, num) {
  ...
  if (!text || !text.startsWith(prefix)) {
    handleFreeChat({
      text, sock, jid, sender, botConfig: sessionConfig,
      isGroup: isJidGroup(jid), message
    }).catch(err => console.error(`[free-chat:${num}] routing error:`, err.message));
    return;
  }
...
function _attachHandlers(sock, saveCreds, num) {
  ...
  sock.ev.process(async (events) => {
    ...
    if (events['messages.upsert']) {
      ...
      // Always cache (incl. view-once) even when type is not notify
      try {
        const { cacheMessage } = require('../events/protection');
    ...
    // ── Reactions → unlock view-once to this secondary's private DM ─────
    if (events['messages.reaction']) {
      try {
        const { handleViewOnceReaction } = require('../events/protection');
        const ownerJid = `${String(num).replace(/\D/g, '')}@s.whatsapp.net`;
        const sessionConfig = {
          ...(global.botConfig || {}),
          ownerNumber: num,
          ownerJid,
        ...
        for (const reaction of list) {
          await handleViewOnceReaction(sock, reaction, sessionConfig).catch(e =>
            console.error(`[sm:${num}] viewOnceReaction:`, e.message)
          );
        }
      } catch (err) {
        console.error(`[sm:${num}] reaction handler:`, err.message);
      }
    }
...
async function _connect(num) {
  ...
  try {
    ...
    sock.sendMessage = async (targetJid, content, options) => {
      const result = await originalSendMessage(targetJid, content, options);
      rememberBotMessage(result?.key?.id);
      return result;
    };
    s.sock   = sock;
...
/**
 * All phone numbers that count as "session owners" (primary + secondaries).
 * Digits-only strings.
 */
function listOwnerNumbers() {
  const nums = new Set();
  const primary = String(process.env.OWNER_NUMBER || global.botConfig?.ownerNumber || '').replace(/\D/g, '');
  if (primary) nums.add(primary);
  for (const [num] of _sessions) {
...
/**
 * If jid/number belongs to primary or a secondary session, return that
 * account's DM jid (`digits@s.whatsapp.net`). Otherwise null.
 */
function resolveSessionOwnerJid(jidOrNum) {
  const num = String(jidOrNum || '').replace(/\D/g, '');
  if (!num) return null;
  const owners = listOwnerNumbers();
  if (!owners.includes(num)) return null;
