        ...
        if (wasConnected) {
          ...
          if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
            ...
            if (!s.notifyJid) {
              ...
              // Use a short exponential backoff (10s, 20s, 40s … cap 120s) so
              // WhatsApp has breathing room before the next pairing attempt.
              s._pairingRetries = (s._pairingRetries || 0) + 1;
              const backoffMs = Math.min(10_000 * Math.pow(2, s._pairingRetries - 1), 120_000);
              console.log(`[sm:${num}] 401 during pairing — stale auth, wiping and restarting in ${Math.round(backoffMs / 1000)}s`);
              await _notify(s,
              ...
              scheduleReconnect(num, backoffMs, true); // freshLogin=true → new code on reconnect
            }
          } else {
...
async function addSession(phoneNumber, { notifyJid, primarySock } = {}) {
  ...
  if (_sessions.has(num)) {
    ...
    // Reconnects for already-known sessions skip the queue — they are not
    // new pairing starts, just restoring a dropped connection.
