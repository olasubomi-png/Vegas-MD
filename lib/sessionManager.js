    ...
    if (events['connection.update']) {
      ...
      if (connection === 'close') {
        ...
        if (wasConnected) {
          ...
          //
          // Mirror main.js logic exactly:
          //
          //   401 loggedOut = corrupt/stale auth keys (previous failed pairing
          //   left partial noise keys that WhatsApp rejects at handshake level).
          //   Fix: wipe session dir so fresh noise keys are generated, reset
          //   pairingCodeRequested, and reconnect → new code sent.
          //
          //   Any other close code = transient network drop while the code
          //   window is open. WhatsApp preserves the pairing session if we
          //   reconnect with the SAME noise keys. Fix: preserve session dir
          //   AND pairingCodeRequested so we silently reconnect without
          //   generating a new code — the existing code stays valid.
          if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
