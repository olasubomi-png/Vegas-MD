    ...
    if (events['connection.update']) {
      ...
      if (connection === 'close') {
        ...
        if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
          console.log(`[sm:${num}] logged out — wiping and reconnecting`);
          try { fs.rmSync(sessionDir(num), { recursive: true, force: true }); } catch (_) {}
          scheduleReconnect(num, RECONNECT_DELAY_MS, true);
        } else if (statusCode === DisconnectReason.forbidden || statusCode === 403) {
          console.log(`[sm:${num}] banned — removing session`);
          removeSession(num);
        } else {
          console.log(`[sm:${num}] disconnected — reconnecting in ${RECONNECT_DELAY_MS}ms`);
...
async function addSession(phoneNumber, { notifyJid, primarySock } = {}) {
  ...
  if (_sessions.has(num)) {
    ...
    // If it exists but isn't connected, update notify targets and reconnect
