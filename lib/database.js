...
class Database {
  ...
  /**
   * Persist DB. Atomic write (tmp + rename) + debounce so concurrent
   * commands do not corrupt database.json.
   * @param {boolean} [immediate=false] flush immediately (e.g. shutdown)
   */
  ...
  _flushSave() {
    ...
    try {
      ...
      try {
        ...
        console.error('[db] fallback save failed:', err2.message);
      }
    }
  }
